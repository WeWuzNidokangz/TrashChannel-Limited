...
import * as DexCalculator from './dex-calculator';
...
class ModdedDex {
	 ...
	 * @return {string}
	 */
	generateMegaStoneName(pokemonName) {
		...
		let lastLetterIsVowel = DexCalculator.isVowel(lastLetter);
		// @ts-ignore
		let penultimateLetter = stoneName.substr( stoneName.length-3, stoneName.length-2 );
		let penultimateLetterIsVowel = DexCalculator.isVowel(penultimateLetter);
	 ...
	 * @return {string}
	 */
	calcActiveAbilitySlot(species, ability) {
		let id = toId(species || '');
		let template = this.getTemplate(id);
	...
	getActiveMove(move: Move | string): ActiveMove {
		...
		let moveCopy: ActiveMove = DexCalculator.deepClone(move);
		moveCopy.hit = 0;
		return moveCopy;
	}
