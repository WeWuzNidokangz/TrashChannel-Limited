...
        },
	...
	mixandmegarule: {
		effectType: 'Rule',
		name: 'Mix And Mega Rule',
		desc: "The mod for Mix And Mega: Mega Stones and Primal Orbs can be used on almost any Pok&eacute;mon with no Mega Evolution limit.",
		mod: 'mixandmega',
		onValidateTeam: function (team) {
			console.log('mixandmegarule: onValidateTeam');
			let itemTable = {};
			for (const set of team) {
				let item = this.getItem(set.item);
				if (!item) continue;
		...
		onValidateSet: function (set, format) {
			console.log('mixandmegarule: onValidateSet');
			let template = this.getTemplate(set.species || set.name);
			let item = this.getItem(set.item);
		...
		onBegin: function () {
			console.log('mixandmegarule: onBegin');
			for (const pokemon of this.p1.pokemon.concat(this.p2.pokemon)) {
				pokemon.originalSpecies = pokemon.baseTemplate.species;
			}
		},
		onSwitchIn: function (pokemon) {
			console.log('mixandmegarule: onSwitchIn');
		...
		onSwitchOut: function (pokemon) {
			console.log('mixandmegarule: onSwitchOut');
			let oMegaTemplate = this.getTemplate(pokemon.template.originalMega);
			if (oMegaTemplate.exists && pokemon.originalSpecies !== oMegaTemplate.baseSpecies) {
...
        onModifyTemplate: function (template, pokemon) {
			...
			for (let statName in template.baseStats) {
				tierShift.baseStats[statName] = DexCalculator.clampIntRange(template.baseStats[statName] + boost, 1, 255);
			}
			if(null != pokemon) {
